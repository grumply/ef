...
{-
Duplication of Oleg Kiselyov's random-shuffle without the MonadRandom
typeclassing to avoid extra dependencies. (people like small build-depends)
-}
module Ef.System.Random.Shuffle (shuffle) where
...
shuffle :: (Lift IO m,Monad m) => [a] -> Pattern fs m [a]
shuffle xs = do
  rng <- lift newStdGen
  return (shuffle'_ xs (length xs) rng)
...
shuffle_ :: [a] -> [Int] -> [a]
shuffle_ elements = shuffleTree (buildTree elements)
    where
...
shuffle'_ elements len = shuffle_ elements . rseq len
    where
