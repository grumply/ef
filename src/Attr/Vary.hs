...
{-# INLINE get #-}
get :: Is (Varying st) fs m => Plan fs m st
get = self (Modify id id)

{-# INLINE gets #-}
gets :: Is (Varying st) fs m => (st -> a) -> Plan fs m a
gets f = self (Modify id f)

{-# INLINE put #-}
put :: Is (Varying st) fs m => st -> Plan fs m ()
put st = self (Modify (const st) (const ()))

{-# INLINE puts #-}
puts :: Is (Varying st) fs m => (a -> st) -> a -> Plan fs m ()
puts f a = self (Modify (const (f a)) (const ()))

{-# INLINE swap #-}
swap :: Is (Varying st) fs m => st -> Plan fs m st
swap st = self (Modify (const st) id)

{-# INLINE modify #-}
modify :: Is (Varying st) fs m => (st -> st) -> Plan fs m ()
