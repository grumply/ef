...
test :: Q [Dec]
test = do
  ds <- mapM reify' [''A,''B,''AB,''CD]
  mon <- lookupName True "Monoid"
  runIO (print (ds,mon))
  return []
--------------------------------------------------------------------------------
-- Test types
...
data A s k = A (s -> k) k
data CoA s k = CoA ((s -> k) -> k)

data B k = B k
data CoB k = CoB k

type AB s = A s Sum.:+: B
type CoAB s = CoA s Product.:*: CoB

data CD k = C (Int -> k) | D k
data CoCD k = CoCD { coC :: (Int,k), coD :: k }

--------------------------------------------------------------------------------
-- Convenience types for manipulating Data/Type declarations and information

newtype Params = Params
  { params :: [Name] }
  deriving (Show,Eq)

newtype Constructors = Constructors
  { constructors :: [(Name,Int)] }
  deriving (Show,Eq)

newtype SubTerm = SubTerm
  { subTerm :: (Maybe Name,Type) }
  deriving (Show,Eq)

newtype CoSubTerm = CoSubTerm
  { coSubTerm :: (Maybe Name,Type) }
  deriving (Show,Eq)

newtype SubTerms = SubTerms
  { subTerms :: [SubTerm] }
  deriving (Show,Eq)

newtype CoSubTerms = CoSubTerms
  { coSubTerms :: [CoSubTerm] }
  deriving (Show,Eq)
...
newtype CoTerm = CoTerm
  { coTerm :: (Name,CoSubTerms) }
  deriving (Show,Eq)

newtype CoTerms = CoTerms
  { coTerms :: [CoTerm] }
  deriving (Show,Eq)
...
reify' :: Name -> Q [(Name,Params,Constructors,Terms)]
reify' nm = do
  i <- reify nm
  monoid <- lookupName True "Monoid"
  case i of
    TyConI d ->
      case d of
        DataD cxt nm params0 cons dervs ->
          let params = []
              constructors = []
              terms = []
          in return [(nm,Params{..},Constructors{..},Terms{..})]
        NewtypeD cxt nm params0 con dervs ->
          let params = []
              constructors = []
              terms = []
          in return [(nm,Params{..},Constructors{..},Terms{..})]
        TySynD nm params0 ty -> do
          params <- undefined
          constructors <- undefined
          terms <- undefined
          return [(nm,Params{..},Constructors{..},Terms{..})]
    x -> error (show x)
...
coizeTerms (Terms ts) = CoTerms (map coizeTerm ts)
  where
    coizeTerm :: Term -> CoTerm
